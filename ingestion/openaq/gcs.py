...
Layout (PROJECT_CONTEXT.md §5): raw/openaq/{country}/{YYYY-MM-DD}/{leaf}.json
where leaf is a sensor id for measurements or the literal "locations" for the
country's location/sensor inventory (sensor ids are numeric, so no collision).
...
def object_name(country_code: str, date: dt.date, leaf: str | int) -> str:
    return f"{RAW_PREFIX}/{country_code}/{date.isoformat()}/{leaf}.json"
...
class RawZoneWriter:
    ...
    def write_pages(
        self, country_code: str, date: dt.date, leaf: str | int, page_bodies: list[str]
    ) -> str:
        if not page_bodies:
            raise ValueError("refusing to write an empty object to the raw zone")
        name = object_name(country_code, date, leaf)

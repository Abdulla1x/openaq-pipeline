...
> **Version:** 1.9 · **Last updated:** 2026-07-17 · **Current phase:** Phase 4 (implemented; live in-Airflow verification pending — needs Docker)
...
**Phase 4 — implemented 2026-07-17 (`feat/phase-4-transformation`); live in-Airflow verification pending (Docker Desktop was unavailable in the session).**
...
- [x] dbt run + test green — locally against live BQ (58/58); to be re-confirmed via the DAG
- [x] 1 exceedance flag hand-verified (two, including a threshold-boundary case)
- [x] Cosmos per-model tasks — 14 tasks (run+test per node) asserted by DAG tests; visible in the UI once live
- [ ] transform DAG triggers off the Dataset — structure asserted in tests; **live trigger needs the compose stack (Docker)**
...
| 1.9 | 2026-07-17 | Phase 4 implemented (`feat/phase-4-transformation`): who_thresholds seed (O3 labeling debt paid, sync test), 3 staging views + int_daily_aqi + two marts (G1/G4/G6/G7/G8) with 51 dbt tests, cosmos 1.15.0 `DbtDag` scheduled on the raw_measurements Dataset via shared `openaq_datasets.py` (G9). Local `dbt build` vs live BQ 58/58 green; dedup halves the duplicated 2026-07-14 batch exactly; two exceedance flags hand-verified from raw JSON (incl. a 15.0-boundary case). SQLFluff now blocking. §7.5: DAG files must not import DAG files; cosmos cache needs the metadata DB. Exit criterion 4 (live Dataset trigger) pending Docker availability. |

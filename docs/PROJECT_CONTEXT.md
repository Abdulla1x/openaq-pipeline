...
- 20 unit tests (`test_client.py` / `test_ingest.py` / `test_gcs.py`), API fully mocked via `responses` (G12), no network or credentials in CI. Runtime deps (`requests`, `google-cloud-storage`) added to `pyproject.toml` with pins matching `airflow/requirements.txt`; CI pytest job installs `-e .`.
- Live manual runs for 2026-07-12 (UTC): **AE** — 31 locations, 52 target sensors, 8 with data / 44 empty, 192 measurements, 9 objects; **PK** — see run record in the PR. Spot-checked one landed object: byte-for-byte the API page body (24 hourly pm25 records).
